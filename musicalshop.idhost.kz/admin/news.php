...
include("include/functions.php");
 ...
 if ($_POST["submit_news"])
 {
    ...
    {
        mysql_query("INSERT INTO news(news_title,news_text,news_date)
            VALUES( 
                            '".$_POST["news_title"]."',
                            '".$_POST["news_text"]."',          
              NOW()                                                                     
...
if (isset($action))
{
   switch ($action) {
      ...
      {
        $delete = mysql_query("DELETE FROM news WHERE news_id = '$id'",$link);  
      }else
      {
...
<p class="links-actions" align="right" ><a class="delete" rel="news.php?id='.$row["news_id"].'&action=delete" >Удалить</a></p>
