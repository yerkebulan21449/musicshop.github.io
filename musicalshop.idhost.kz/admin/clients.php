...
if (isset($action))
{
   switch ($action) {
      ...
      if ($_SESSION['delete_clients'] == '1')
      {
         ...
         $delete = mysql_query("DELETE FROM users WHERE users_id = '$id'",$link);      
...
if ($temp[0] > 0)   
{ 
$result = mysql_query("SELECT * FROM users ORDER BY users_id DESC LIMIT $start, $num",$link);
...
do
{
 ...
 <p class="client-datetime" >'.$row["users_datetime"].'</p>
 <p class="client-email" ><strong>'.$row["users_mail"].'</strong></p>
 <p class="client-links" ><a class="delete" rel="clients.php?id='.$row["users_id"].'&action=delete" >Удалить</a></p>
 
 
 <ul>
 <li><strong>E-Mail</strong> - '.$row["users_mail"].'</li>
 <li><strong>ФИО</strong> - '.$row["users_surname"].' '.$row["users_name"].'</li>
 <li><strong>Адрес</strong> - '.$row["users_address"].'</li>
 <li><strong>Телефон</strong> - '.$row["users_phone"].'</li>
 <li><strong>IP</strong> - '.$row["users_ip"].'</li>
 <li><strong>Дата регистрации</strong> - '.$row["users_datetime"].'</li>
 <li><strong>Логин</strong> - '.$row["users_login"].'</li>
 <li><strong>Пароль</strong> - '.$row["users_password"].'</li>
 </ul>
