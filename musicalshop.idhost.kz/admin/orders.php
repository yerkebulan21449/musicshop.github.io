...
<?php
 ...
 $all_count = mysql_query("SELECT * FROM zakaz",$link);
 $all_count_result = mysql_num_rows($all_count);

$novie = mysql_query("SELECT * FROM zakaz where zakaz_confirm = ''",$link);
 $novie_result = mysql_num_rows($novie);

 $buy_count = mysql_query("SELECT * FROM zakaz WHERE zakaz_confirm = 'yes'",$link);
 $buy_count_result = mysql_num_rows($buy_count);

 $no_buy_count = mysql_query("SELECT * FROM zakaz WHERE zakaz_pay = 'cancelled'",$link);
...
<li>Новые - <strong><? echo  $novie_result; ?></strong></li>
...
<?php
  $result = mysql_query("SELECT * FROM zakaz ORDER BY $sort",$link);
...
} else
{
 ...
 <div class="block-order">
  ...
  <p class="order-link" ><a class="green" href="view_order.php?id='.$row["zakaz_id"].'" >Подробнее</a></p>
 </div>
 ';   
