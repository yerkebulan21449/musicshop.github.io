...
if (isset($action))
{
   ...
   if ($_SESSION['accept_reviews'] == '1')
   {
        ...
        $update = mysql_query("UPDATE reviews SET reviews_moderate='1' WHERE reviews_id = '$id'",$link);  
   ...
   if ($_SESSION['delete_reviews'] == '1')
   {
        ...
        $delete = mysql_query("DELETE FROM reviews WHERE reviews_id = '$id'",$link);      
...
<?php
 ...
 $no_accept_count = mysql_query("SELECT * FROM reviews WHERE reviews_moderate = '0'",$link);
...
$result = mysql_query("SELECT * FROM reviews,items WHERE items.items_id = reviews.items_id ORDER BY $sort LIMIT $start, $num",$link);
...
if ($row["reviews_moderate"] == '0'){ $link_accept = '<a class="green" href="reviews.php?id='.$row["reviews_id"].'&action=accept" >Принять</a> | ';  } else { $link_accept = '';  }
...
<p class="author-date"><strong>'.$row["name"].'</strong>, '.$row["reviews_date"].'</p>
